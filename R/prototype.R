#install.packages("VA", repos="http://gking.harvard.edu", lib="~/.R/library")
...
readme <- function(undergradlist = list(), trainingset = NULL, testset =
                   NULL, formula = NULL, features = NULL, n.subset =
                   NULL, prob.wt = NULL, boot.se = NULL, nboot = NULL, printit = NULL)
{
  if(is.null(testset))
...
preprocess <- function(undergrad.results){
  ...
  if(length(remove.idx)>0){
    ...
    txt <- paste("undergrad.preprocessed$formula <- ",colnames(undergrad.preprocessed$trainingset)[4],"+ ... +",colnames(undergrad.preprocessed$trainingset)[nnn],"~TRUTH",sep="")
    eval(parse(text=txt))
  }else{
